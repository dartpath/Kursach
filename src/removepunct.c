...
int remove_punctuation_with_word_counting(
        char* Origin_text, int Word_start_numbers[])
{
    int Beginning_of_the_word, i, Word_count = 0;

    for (Word_count = 0, Beginning_of_the_word = 1, i = 0; Origin_text[i];
         i++) {
        if (Origin_text[i] == ' ' || Origin_text[i] == ':'
